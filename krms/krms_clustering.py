# -*- coding: utf-8 -*-
"""
Created on Mon Oct 19 10:15:33 2020
...
def returnCluster(data, dimensions):
    '''
...
def run(file="nothing"):
    ...
    try:
        ...
        for col in data.columns:
            if type(data[col].values[0])!=type(7.0) or type(data[col].values[0])!=type(7):
                data[col] = labelencoder.fit_transform(data[col])
        N_cols=len(data.columns)-1
        dims = N_cols
        Y = list(data.iloc[:, 0].values)
        num_clusters = len(list(set(Y)))  
        points=[Point(i[:dims]) for i in data.iloc[:, [it for it in range(1,N_cols+1)]].values]
        dimensions=dims
    except:
        iris=load_iris()
        X=iris.data
...
#    try:
#        print(returnCluster(best_clusters, dimensions))
#    except:
#        pass
    z=[]
